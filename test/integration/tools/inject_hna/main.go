...
func main() {
	...
	fmt.Println("=== Injecting HNA Prefix 192.168.10.0/24 into R1 ===")
	sendReq("POST", "http://localhost:8081/api/v1/hna", map[string]string{"prefix": "192.168.10.0/24"})

	fmt.Println("=== Waiting 6 seconds for HNA propagation ===")
	time.Sleep(6 * time.Second)

	fmt.Println("=== Querying R1 local HNAs ===")
	sendReq("GET", "http://localhost:8081/api/v1/hna", nil)

	fmt.Println("=== Querying R2 remote HNAs ===")
	sendReq("GET", "http://localhost:8082/api/v1/hna", nil)
